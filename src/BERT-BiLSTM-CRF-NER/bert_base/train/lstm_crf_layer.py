...
class BLSTM_CRF(object):
    def __init__(self, embedded_chars, hidden_unit, cell_type, num_layers, dropout_rate,
                 initializers, num_labels, seq_length, labels, lengths, is_training, constraint=None):
        """
        BLSTM-CRF 网络
        ...
        if constraint is not None:
            ...
            for i, c in enumerate(constraint):
                constraint_1.append(tf.get_variable("constraint_{}".format(i), initializer=c, trainable=False))
            self.constraint = constraint_1
        else:
    ...
    # def _create_sentence_level_decode(self, logits, trans, norm=False):
    #     bsz = tf.shape(logits)[0]
    #     lsz = len(self.labels)
    ...
    def crf_layer(self, logits):
        ...
        with tf.variable_scope("crf_loss"):
            ...
            if self.constraint is not None:
                ...
                trans = tf.add(tf.multiply(trans, self.mask), self.inv_mask, name="transitions")
